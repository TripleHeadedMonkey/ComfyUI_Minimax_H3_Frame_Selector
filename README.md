...
When an off-grid selection places the endpoint anchor on the first saveable frame, that picture duplicates the preceding accepted clip's final frame. The selector removes only this repeated video frame (`video_start=40` for a 39-frame handover) while audio continues from frame 39. The timeline advances by the actual saved video count, preventing a one-frame motion pause without skipping the guidance track.
