...
- **Save Project Start Frame** is an output node placed after the selector. It saves the first incoming image as `frame_000001.png`, `frame_000002.png`, and so on inside `ComfyUI/output/frame_selector_projects/<project>/start_frames/`.

The default `frame_index` is `-1`, so normal repeated runs always use the newest saved frame. After a discard, the newest frame is unchanged and is therefore reused automatically. Changing the index lets you scroll through earlier scenes without any third mode or cursor state. Give both nodes exactly the same `project_name`. Connecting the selector's selected image to the save node means **Discard Generation** blocks the save automatically.
